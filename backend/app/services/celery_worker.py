...
from io import StringIO # <-- Important for reading strings as files
        ...
        for ws in workspaces_to_check:
            ...
            if is_due:
                if ws.data_source == 'API' and ws.api_url:
                    logger.info(f"✅ API DUE! Triggering API fetch for '{ws.name}'.")
                    fetch_api_data.delay(str(ws.id))
                elif ws.data_source == 'DB' and ws.db_host and ws.db_query:
                    logger.info(f"✅ DB DUE! Triggering DB fetch for '{ws.name}'.")
                    fetch_db_data.delay(str(ws.id))
    finally:
        db.close()

# ====================================================================
#  The "API Fetcher Robot" - UPGRADED
# ====================================================================
...
def fetch_db_data(workspace_id: str):
    ...
    try:
        ...
        logger.info(f"-> [DB FETCHER] Connecting to {workspace.db_host}...")
        ...
        try:
            ...
            if triggered:
                ...
                email_context = {
                    ...
                    "rule": { "column_name": rule.column_name, "metric": rule.metric, "condition": rule.condition, "value": rule.value },
...
# ====================================================================
#  The "Analyzer Robot" - FINAL, UPGRADED VERSION
# ====================================================================
...
def process_csv_task(upload_id: str):
    ...
    try:
        ...
        if not current_upload:
            logger.warning(f"Could not find DataUpload record for ID: {upload_id}. Aborting.")
            return

        csv_content = current_upload.file_content
        if not csv_content:
            logger.warning(f"No file content found for upload ID: {upload_id}. Aborting.")
            return
        ...
        if previous_upload and previous_upload.analysis_results:
            if previous_upload.upload_type == current_upload.upload_type:
                ...
                if old_row_count is not None and old_row_count != new_row_count:
        ...
        if workspace:
            if schema_has_changed or row_count_has_changed:
                ...
                email_context = {
                    "workspace_name": workspace.name, "upload_type": current_upload.upload_type,
                    "new_file_name": current_upload.file_path, "old_file_name": previous_upload.file_path if previous_upload else "N/A",
                    ...
                    "metric_changes": {'old_rows': old_row_count, 'new_rows': new_row_count, 'percent_change': f"{((new_row_count - old_row_count) / old_row_count) * 100 if old_row_count != 0 else 0:+.1f}%"} if row_count_has_changed else {}
                }
                recipients = [user.email for user in users_to_notify]
        ...
        redis_client.publish(f"workspace:{str(workspace_id)}", "job_complete")
        ...
        if upload:
            redis_client.publish(f"workspace:{str(upload.workspace_id)}", "job_error")
        return {"status": "error", "message": str(e)}
    finally:
        db.close()
        
# ====================================================================
#  NEW: The "OTP Email Chef"
# ====================================================================
@celery_app.task(name="send_otp_email_task")
def send_otp_email_task(to_email: str, otp: str, subject_type: str):
    """
    A background task to send the OTP/Password Reset email.
    """
    logger.info(f"📨 [WORKER] Preparing to send OTP email to {to_email}...")
    # We use asyncio.run because the email function is async, but this Celery task is sync.
